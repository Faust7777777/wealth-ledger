...
- `crypto-instruments/ensure` 登记或复用用户消息、附件或查询结果中实际出现的加密资产代码，并补齐账户的报价币种支持；服务端规范化代码并生成真实 `instrumentId`，不接受模型自造 ID。BTC/ETH/USDT 可按账户默认币种使用内置 CoinGecko 路径；其他新登记 crypto 固定以 USDT 作为标的计价币种，与账户展示币种解耦，再通过已保存的 FX 路径折算到 CNY/USD 等展示单位。它不创建持仓、余额、报价或 movement。Pi Agent 的加密资产快照工具只让模型提交来源中的 `symbol + targetQuantity`，sidecar 必须先调用 ensure、完整校验返回的 symbol/ID 映射，再把真实 ID 提交给持仓快照接口；缺少任一映射时不得提交快照。快照仍只进入待审核。内置 public provider 只为其明确支持的交易对提供结构化行情；没有报价时保留原始数量并逐项报告缺报价。
- `investment-instruments/ensure` 用于券商、基金平台或其他非 crypto 持仓来源。每项必须明确提供来源中出现的 `type + symbol + market + quoteCurrency + displayName`，且不接受 `id`；Rust 按类型、市场和代码严格匹配，复用或生成真实 `instrumentId`，并补齐账户支持的计价币种。已有相同身份但计价币冲突时整次失败，不静默改写。它同样只写标的与账户元数据，不创建数量、报价或 movement；Agent 必须使用返回的完整真实 ID 映射再提交一个待审核快照。
