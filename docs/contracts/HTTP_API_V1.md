...
POST /v1/accounts/{accountId}/investment-instruments/ensure
...
- `crypto-instruments/ensure` 登记或复用用户消息、附件或查询结果中实际出现的加密资产代码，并补齐账户的报价币种支持；服务端规范化代码并生成真实 `instrumentId`，不接受模型自造 ID。BTC/ETH/USDT 可按账户默认币种使用内置 CoinGecko 路径；其他新登记 crypto 固定以 USDT 作为标的计价币种，与账户展示币种解耦，再通过已保存的 FX 路径折算到 CNY/USD 等展示单位。它不创建持仓、余额、报价或 movement。Pi Agent 的加密资产快照工具只让模型提交来源中的 `symbol + targetQuantity`，sidecar 必须先调用 ensure、完整校验返回的 symbol/ID 映射，再把真实 ID 提交给持仓快照接口；缺少任一映射时不得提交快照。快照成功后 sidecar 必须对正数量标的及其到账户展示币种的 FX 路径执行一次结构化 lookup，并保存为独立 `suggested` 报价候选；查询失败不得把已持久化的快照误报为失败或触发快照重试。快照与报价仍分别审核，任何一方都不自动采用。内置 public provider 只为其明确支持的交易对提供结构化行情；没有报价时保留原始数量并逐项报告缺报价。
- `investment-instruments/ensure` 用于券商、基金平台或其他非 crypto 持仓来源。每项必须明确提供来源中出现的 `type + symbol + market + quoteCurrency + displayName`，且不接受 `id`；Rust 按类型、市场和代码严格匹配，复用或生成真实 `instrumentId`，并补齐账户支持的计价币种。已有相同身份但计价币冲突时整次失败，不静默改写。校验错误与匹配冲突必须保留原始 `instruments[index]` 下标；Agent sidecar 必须把有界的逐项诊断传回模型，不能只保留笼统错误码后让模型猜测。它同样只写标的与账户元数据，不创建数量、报价或 movement；Agent 必须使用返回的完整真实 ID 映射再提交一个待审核快照，并在快照成功后按上一条相同规则生成独立的标的与 FX 报价候选。
...
- 最新估值刷新可显式配置 `FINWEALTH_QUOTE_PROVIDER=public`：BTC/ETH/USDT 使用 CoinGecko，其他已登记且以 USDT 计价的 crypto 标的使用 OKX 公共现货 ticker，已登记 equity/fund 按明确 market 映射到 Yahoo 当前行情，传统法币 FX 使用 Frankfurter/ECB；默认 `none` 不联网。股票/基金必须携带受支持的明确市场，Yahoo 返回币种必须与标的计价币一致，单项失败不得阻断其他目标。结构化 lookup 只读，Agent 仍须把结果提交为待审核报价候选，采用前不改变估值。`public` 不提供历史行情，历史价格仍只在独立 Yahoo provider 下可用。
- 单次结构化 lookup 最多并行 8 个标的 provider 请求并保持输入顺序；该上限适用于最多 100 项的持仓快照自动查价，避免串行超时和无界并发。
...
- Pi Agent 必须通过专用工具读取服务端的固定收益/贷款应计结果与还款计划，再调用专用 proposal 入口；不得自行计算本金、利率或利息金额。Agent 只能生成收益利息、贷款利息或“利息+还款”待审核组，不能调用确认接口，也不能在确认前宣称余额已变化。收益或贷款条款尚未配置时应报告缺少条款，不得猜测。
