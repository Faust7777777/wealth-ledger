...
- `crypto-instruments/ensure` 登记或复用用户消息、附件或查询结果中实际出现的加密资产代码，并补齐账户的报价币种支持；服务端规范化代码并生成真实 `instrumentId`，不接受模型自造 ID。它不创建持仓、余额、报价或 movement。Pi Agent 的加密资产快照工具只让模型提交来源中的 `symbol + targetQuantity`，sidecar 必须先调用 ensure、完整校验返回的 symbol/ID 映射，再把真实 ID 提交给持仓快照接口；缺少任一映射时不得提交快照。快照仍只进入待审核。内置 public provider 只为其明确支持的交易对提供结构化行情；没有报价时保留原始数量并逐项报告缺报价。
