...
- `investment-instruments/ensure` 用于券商、基金平台或其他非 crypto 持仓来源。每项必须明确提供来源中出现的 `type + symbol + market + quoteCurrency + displayName`，且不接受 `id`；Rust 按类型、市场和代码严格匹配，复用或生成真实 `instrumentId`，并补齐账户支持的计价币种。已有相同身份但计价币冲突时整次失败，不静默改写。它同样只写标的与账户元数据，不创建数量、报价或 movement；Agent 必须使用返回的完整真实 ID 映射再提交一个待审核快照，并在快照成功后按上一条相同规则生成独立的标的与 FX 报价候选。
