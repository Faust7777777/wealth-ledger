...
- 后端分支：`feat/pi-agent-control-center`；开始前以远端最新 HEAD 为基线新建独立工作树。
...
4. 消息发送后立即显示 queued 状态，以 SSE 合并 `message.delta`、`tool.started`、`tool.completed`、`run.completed`、`run.failed`。
...
4. fixture/MockClient 不得伪造生产写入成功；真实 smoke 至少验证 Rust 代理可达、会话持久化、附件上传、无模型时 503 fail-closed。
