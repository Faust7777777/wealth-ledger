...
try {
  ...
  try {
    ...
    & flutter test "--dart-define=LOCAL_SERVER_API_BASE=$baseUrl" --concurrency=1 `
      ...
      test/local_server_ai_image_integration_test.dart
  ...
  Write-Host "OK: Flutter local-server integration smoke passed (subscriptions + accounts + dca + trades + holdings + loans + ai-text + ai-image)"
} catch {
